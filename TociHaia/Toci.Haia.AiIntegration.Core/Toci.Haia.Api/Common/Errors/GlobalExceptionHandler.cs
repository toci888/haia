    ...
    IProblemDetailsService problemDetailsService) : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext httpContext, Exception exception, CancellationToken cancellationToken)
    {
        ...
        logger.LogError(exception, "Request failed with code {Code}", code);
    ...
    private static (HttpStatusCode status, string code, string title, string detail) Map(Exception ex)
    {
        if (ex is DomainException de)
        {
            return (de.StatusCode, de.Code, "Domain error", de.Message);
        }
